...
dtype_dict = {
    "string": ArrayType(StringType(), True),
    "dense": ArrayType(FloatType(), True),
    "sparse": ArrayType(IntegerType(), True),
}
FIELDS = [
    StructField(key, dtype_dict[value["value_type"]]) for key, value in config.items()
]
...
if __name__ == "__main__":
    ...
    # mdfs://cloudhdfs/newspluginalgo/user/pinnie/model_recall/bottom_add_feature/neg_sample/%YYYYMMDDHH% mdfs://cloudhdfs/newspluginalgo/user/pinnie/model_recall/bottom_add_feature/tfrecord/%YYYYMMDDHH%
    ...
    features_rdd = features_rdd.map(lambda x: x.split("\t")).filter(
        lambda x: len(x) >= 152
    )
    ...
    tf_in_rdd = features_rdd.map(parse_for_tfrecord).filter(
    ...
    df.repartition(200).write.format("tfrecords").option("recordType", "Example").\
        option("codec", "org.apache.hadoop.io.compress.GzipCodec").save(output_root + "/train.tfrecord")
