...
def add_freq_to_data(line: str) -> str:
    ...
    FREQ = config_realtime["item_freq"]["feature_index"]  # index of item frequency (position to fill in)
    cmsid = fields[ID]
...
if __name__ == "__main__":
    ...
    # mdfs://cloudhdfs/mttsparknew/data/szrecone/PCG/newshotalgo/user/shueli/landing_bottom_realtime/sample_dedup/%YYYYMMDDHH% mdfs://cloudhdfs/newspluginalgo/user/pinnie/model_recall/bottom_realtime/only_item_feature 4 mdfs://cloudhdfs/newspluginalgo/user/pinnie/model_recall/bottom_realtime/neg_sample/%YYYYMMDDHH% %YYYYMMDDHH%
    ...
    for x in item_info_rdd.take(5):
        print(x)
    item_info_rdd = item_info_rdd.map(lambda line: split_item_line(line)).reduceByKey(lambda x, y: x)
    # print("item info rdd count is: %d" % item_info_rdd.count())
    print("item_info_rdd after split_item_line  count: {}".format(item_info_rdd.count()))
    for x in item_info_rdd.take(5):
        print(x)
    ...
    original_sample_rdd = original_sample_rdd.map(lambda line: line_split_and_gen_label(line))
    pos_sample_rdd = original_sample_rdd.filter(lambda larr: len(larr) > 102 and float(larr[17]) == 1.0)
    pos_sample_rdd_count = pos_sample_rdd.count()
    print("pos_sample_rdd_count :%d" % pos_sample_rdd_count)

    neg_sample_rdd = original_sample_rdd.filter(lambda larr: len(larr) > 102 and float(larr[17]) == 0.0)
    ...
    neg_sample_rdd = neg_sample_rdd.map(lambda line: neg_sample_with_ratio(line, neg_sample_ratio)).filter(
        lambda x: x != None).persist(StorageLevel.MEMORY_AND_DISK)
    print("neg_sample_rdd count:", neg_sample_rdd.count())

    sampled_rdd = pos_sample_rdd.flatMap(lambda larr: sample_item(int(sample_num / 2), larr)).persist(StorageLevel.MEMORY_AND_DISK)
    ...
    for x in sampled_rdd.take(5):
        ...
        for key, value in iterm_key_values.items():
            print_line += "{}:{}:feature_index: {}".format(key, x1[value["feature_index"]], value["feature_index"])
        print(print_line)
    ...
    sampled_all_with_freq = sampled_all.map(lambda x: add_freq_to_data(x)).persist(StorageLevel.MEMORY_AND_DISK)
    for x in sampled_all_with_freq.take(10):
        print(x)

    FREQ = config_realtime["item_freq"]["feature_index"]  # index of item frequency (position to fill in)
    debug_sample = sampled_all_with_freq.map(lambda x: x.split('\t')).map(lambda x: (x[ID], x[FREQ]))
