...
def run(args):
    ...
    if args.estimator_mode == "export":
        ...
        data_files = data_files[2:120 * 10]
        LOG("data_files in predict: {}".format(data_files))
        dssm_input_fn = lambda: input_fn(
            file_names=data_files,
            label_name=label_name,
            data_format=data_format,
        ...
        import numpy as np
        predict_dir = os.path.join(os.path.join(args.model_dir, "user_item_key_values"))
        try:
            LOG("Try to mkdir: {}".format(predict_dir))
            os.mkdir(predict_dir)
        except:
            LOG("Failed to mkdir predict dir: {}".format(predict_dir))
        LOG("start to predict user and item vector!")
        user_key_file = os.path.join(predict_dir, "user_key.npy")
        user_value_file = os.path.join(predict_dir, "user_value.npy")
        item_key_file = os.path.join(predict_dir, "item_key.npy")
        item_value_file = os.path.join(predict_dir, "item_value.npy")
        user_keys = []
        user_values = []
        item_keys = []
        item_values = []
        user_start = 0
        item_start = 0
        for x in DSSM.predict(input_fn=dssm_input_fn, yield_single_examples=False):
            user_ids_array = x["user_id"].astype(str)
            user_ids_unique, user_ids_index, user_ids_counts = np.unique(user_ids_array, return_index=True, return_counts=True)
            user_ids_index_dict = dict(zip(user_ids_unique, user_ids_index))
            user_ids_counts_dict = dict(zip(user_ids_unique, user_ids_counts))
            user_ids_new_index = np.array([value for key, value in user_ids_index_dict.items() if key not in user_ids_counter])
            user_ids_counts_dict = {key: int(value + user_ids_counter.get(key, 0)) for key, value in user_ids_counts_dict.items()}
            user_ids_counter.update(user_ids_counts_dict)
            if user_ids_new_index.shape[0] > 0:
                user_ids_array = user_ids_array[user_ids_new_index]
                user_keys.append(x["user_norm"][user_ids_new_index])
                user_values.append(user_ids_array)


            item_ids_array = x["item_newsid"].astype(str)
            item_ids_unique, item_ids_index, item_ids_counts = np.unique(item_ids_array, return_index=True, return_counts=True)
            item_ids_index_dict, item_ids_counts_dict = dict(zip(item_ids_unique, item_ids_index)), dict(zip(item_ids_unique, item_ids_counts))
            item_ids_new_index = np.array([value for key, value in item_ids_index_dict.items() if key not in item_ids_counter])
            item_ids_counts_dict = {key: int(value + item_ids_counter.get(key, 0)) for key, value in item_ids_counts_dict.items()}
            item_ids_counter.update(item_ids_counts_dict)
            if item_ids_new_index.shape[0] > 0:
                item_keys.append(x["item_norm"][item_ids_new_index])
                item_ids_array = item_ids_array[item_ids_new_index]
                item_values.append(item_ids_array)

            user_start += user_ids_new_index.shape[0]
            item_start += item_ids_new_index.shape[0]
            if user_start > args.max_user_cnt:
                print("meet max user count: {}".format(args.max_user_cnt))
                break
            print("user cnt is {}, item cnt is: {}".format(user_start, item_start))
        print("total user cnt is {}, item cnt is: {}".format(user_start, item_start))
        assert len(user_ids_counter) == user_start
        assert len(item_ids_counter) == item_start
        user_item_counter_file = os.path.join(predict_dir, "user_item_counter.json")
        print("save user item counter to: {}".format(user_item_counter_file))
        with open(user_item_counter_file, "w") as f:
            json.dump({"user_ids_counter": user_ids_counter, "item_ids_counter": item_ids_counter}, f)

        item_keys = np.concatenate(item_keys, axis=0)
        item_values = np.concatenate(item_values, axis=0)
        LOG("save key to: {}".format(item_key_file))
        LOG("save value to: {}".format(item_value_file))
        np.save(item_key_file, item_keys)
        np.save(item_value_file, item_values)

        user_keys = np.concatenate(user_keys, axis=0)
        user_values = np.concatenate(user_values, axis=0)
        LOG("save key to: {}".format(user_key_file))
        LOG("save value to: {}".format(user_value_file))
        np.save(user_key_file, user_keys)
        np.save(user_value_file, user_values)
        LOG("DONE PREDICT")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gzip", default=0, type=int, help="if to use gzip")
    parser.add_argument("--cl_u2u_rate", default=0.0, type=float, help="weight of cl loss")
    parser.add_argument("--cl_i2i_rate", default=0.0, type=float, help="weight of cl loss")
    parser.add_argument("--cl_u2i_rate", default=0.0, type=float, help="weight of cl loss")
    parser.add_argument("--cl_dropout_rate", default=0.0, type=float, help="weight of cl loss")
    parser.add_argument("--greater_30_weight", default=0.0, type=float, help="weight of greater 30")
    parser.add_argument("--watch_time_th", default=10, type=float, help="weight of greater 30")
    parser.add_argument("--max_user_cnt", default=20000000, type=int, help="max user number for predict")
