        ...
        for ky, value in list(user_ids.items()) + list(item_ids.items()):
            ...
            if "value_type" in value and value["value_type"] == "dense":
                ...
                if filter_default:
                    ...
                    if "embedding_weight" in value and value["embedding_weight"] and value["embedding_weight"] in features:
                        # [bs, len]
...
def fixed_size_emb_lookup_cl(user_ids, item_ids, features, filter_default, params, cl_dropout_rate=0.0):
    """
    :param user_ids:
    :param item_ids:
    :param features:
    ...
    with tf.variable_scope("user_and_item_emb", reuse=tf.AUTO_REUSE):
        ...
        for ky, value in list(user_ids.items()) + list(item_ids.items()):
            ...
            if "value_type" in value and value["value_type"] == "dense":
                ...
                # sparse feature
                if value.get("value_type", "") == "string":
                    LOG("sparse feature: {}".format(ky))
                    # if value["field_size"] > 1 and ("item" in ky or "user_history" in ky or "user_realtime_pos_video" in ky):
                    if value["field_size"] > 1:
                        feat_dnn_ids_mask = tf.not_equal(feat_dnn_ids, tf.constant("pad"),
                                                         name='{}_mask'.format(ky))
                        feat_dnn_ids_mask_float = tf.cast(tf.expand_dims(feat_dnn_ids_mask, axis=2),
                                                          dtype=tf.float32, name='{}_mask_float'.format(ky))
                ...
                if filter_default:
                    ...
                    if value['field_size'] > 1:
                        LOG("=========================mask float============{}".format(ky))
                        if cl_dropout_rate > 0.0:
                            LOG("======================mask cl dropout rate: {}".format(cl_dropout_rate))
                            mask_cl = tf.random_uniform(tf.shape(feat_dnn_ids_mask_float)) + (1 - cl_dropout_rate)
                            mask_cl = tf.floor(mask_cl)
                            # self.mask_cl = tf.Print(self.mask_cl, [self.mask_cl], "self.mask_cl: ", first_n=100, summarize=128)
                            # self.mask = tf.Print(self.mask, [self.mask], "self.mask: ", first_n=100, summarize=128)
                            feat_dnn_ids_mask_float = mask_cl * feat_dnn_ids_mask_float
                        lookup_filtered = lookup_filtered * feat_dnn_ids_mask_float
                        mask_sum = tf.expand_dims(
                            tf.reduce_sum(tf.cast(feat_dnn_ids_mask, dtype=tf.float32), axis=1, keep_dims=True), axis=2)
                        lookup = tf.reduce_sum(lookup_filtered, axis=1, keep_dims=True)
                    else:
                        lookup = tf.reduce_sum(lookup_filtered, axis=1, keep_dims=True)
                else:
                    raise NotImplementedError("NOT SUPPORT SPARSE FEATURE")
...
def add_time_weight_to_loss_for_bottom_page(loss, label, watch_time, greater_30_weight=0.1):
    greater_30 = tf.cast(tf.greater_equal(watch_time, 30), tf.float32, name="greater_30")
    loss_ret = loss * (greater_30 * greater_30_weight + 1)
    return loss_ret
