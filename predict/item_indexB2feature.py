...
def getNewsInfo_index(line):
    try:
        ...
        for key, value in ITEM_FIELDS_DICT.items():
            ...
            elif key == "item_concept_tags":
                index_value = newsinfo.get(index_name, {})
                item_concept_tags = list(index_value.keys())
                item_concept_tags = ",".join(item_concept_tags)
                feature_line[feature_index] = item_concept_tags
            else:
                index_value = newsinfo.get(index_name, "")
...
def get_newest_2file(sc, input_path):
    URI = sc._gateway.jvm.java.net.URI
    Path = sc._gateway.jvm.org.apache.hadoop.fs.Path
    FileSystem = sc._gateway.jvm.org.apache.hadoop.fs.FileSystem
    FileStatus = sc._gateway.jvm.org.apache.hadoop.fs.FileStatus
    ...
    listdir = fs.listStatus(Path(input_path))
    if len(listdir) > 2:
        return [listdir[-1].getPath().toString(), listdir[-2].getPath().toString()]
    else:
...
if __name__ == "__main__":
    ...
    # mdfs://cloudhdfs/newsrecomindex/data/offlineindex/IndexB/ mdfs://cloudhdfs/newspluginalgo/user/pinnie/model_recall/bottom_add_feature/only_item_feature/%YYYYMMDDHH%
    ...
    day = get_newest_file(sc, index_root_list[0])
    _, hour = get_newest_2file(sc, day)
    indexfile = os.path.join(hour, "video")
